...
<body>
	...
	<div class="row">
					...
					<div class="col-md-1">
							<button class="btn btn-primary float-right" type="button" onclick="location.href='reviewwrite'">글쓰기</button> 
					</div>
					<div>
						<nav aria-label="pagenav">
							<ul class="pagination justify-content-center">
							<c:if test="${pageMaker.prev }">
								<li class="page-item"><a class="page-link" href='<c:url value="community_review?page=${pageMaker.startPage-1 }"/>'>이전</a></li>
							</c:if>		
							<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="pageNum">
								<li class="page-item"><a class="page-link" href='<c:url value="community_review?page=${pageNum }"/>'>${pageNum }</a>
								</li>							
							</c:forEach>	
							<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
								 <li class="page-item"><a class="page-link"	href='<c:url value="community_review?page=${pageMaker.endPage+1 }"/>'>다음</a>
								</li>
							</c:if>
							</ul>
						</nav>
